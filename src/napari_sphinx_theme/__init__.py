...
THEME_PATH = (Path(__file__).parent / "theme" / "napari-sphinx-theme").resolve()
...
def _html_page_context(
    ...
    if app.config.html_theme != "napari":
        return

    if "css_files" in context:
        if "_static/styles/napari-sphinx-theme.css" not in context["css_files"]:
            raise Exception(
                "This documentation is not using `napari-sphinx-theme.css` as the stylesheet. "
                "If you have set `html_style` in your conf.py file, remove it."
            )

        _add_asset_hashes(
            context["css_files"],
            ["styles/napari-sphinx-theme.css", "styles/furo-extensions.css"],
        )
    if "scripts" in context:
        _add_asset_hashes(
            context["scripts"],
            ["scripts/napari-sphinx-theme.js"],
        )
...
def _builder_inited(app: sphinx.application.Sphinx) -> None:
    ...
    app.add_js_file("scripts/napari-sphinx-theme.js", priority=200)
